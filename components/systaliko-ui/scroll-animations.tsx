...
interface ScrollTransformProps extends HTMLMotionProps<"div"> {
  xRange?: unknown[];
  yRange?: unknown[];
  scaleRange?: unknown[];
  rotateRange?: unknown[];
...
export function ScrollTransform({
  ...
  const { x, y, scale, rotate } = useTransform(
    scrollProgress,
    inputRange,
    {
      x: xRange,
  ...
  const transform = useMotionTemplate`translate(${x}px, ${y}px) scale(${scale}) rotate(${rotate}deg)`;
  return <motion.div style={{ transform, ...style }} {...props} />;
}
